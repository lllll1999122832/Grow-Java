...
Redis 官网相关介绍 [https://redis.io/topics/transactionsopen in new window](https://redis.io/topics/transactions) 如下：

![Redis 事务](https://oss.javaguide.cn/github/javaguide/database/redis/redis-transactions.png)Redis 事务

### [#](#redis-事务支持原子性吗) Redis 事务支持原子性吗？
...
![Redis 为什么不支持回滚](https://oss.javaguide.cn/github/javaguide/database/redis/redis-rollback.png)Redis 为什么不支持回滚

**相关 issue** :

- [issue#452: 关于 Redis 事务不满足原子性的问题open in new window](https://github.com/Snailclimb/JavaGuide/issues/452) 。
- [Issue#491:关于 Redis 没有事务回滚？open in new window](https://github.com/Snailclimb/JavaGuide/issues/491)
...
## [#](#redis-性能优化-重要) Redis 性能优化（重要）

除了下面介绍的内容之外，再推荐两篇不错的文章：

- [你的 Redis 真的变慢了吗？性能优化如何做 - 阿里开发者open in new window](https://mp.weixin.qq.com/s/nNEuYw0NlYGhuKKKKoWfcQ)
- [Redis 常见阻塞原因总结 - JavaGuideopen in new window](https://javaguide.cn/database/redis/redis-common-blocking-problems-summary.html)
...
>
> 我在 [Redis 集群详解（付费）open in new window](https://javaguide.cn/database/redis/redis-cluster.html) 这篇文章中详细介绍了 Redis Cluster 这部分的内容，感兴趣地可以看看。
...
![img](https://oss.javaguide.cn/github/javaguide/database/redis/redis-pipeline-vs-transaction.png)
...
### [#](#redis-bigkey-大-key) Redis bigkey（大 Key）

#### [#](#什么是-bigkey) 什么是 bigkey？
...
![阿里云Key分析](https://oss.javaguide.cn/github/javaguide/database/redis/aliyun-key-analysis.png)阿里云Key分析
...
### [#](#redis-hotkey-热-key) Redis hotkey（热 Key）
...
京东零售的 [hotkeyopen in new window](https://gitee.com/jd-platform-opensource/hotkey) 这个项目不光支持 hotkey 的发现，还支持 hotkey 的处理。

![京东零售开源的 hotkey](https://oss.javaguide.cn/github/javaguide/database/redis/jd-hotkey.png)京东零售开源的 hotkey
...
![通过阿里云的Proxy Query Cache优化热点Key问题](https://oss.javaguide.cn/github/javaguide/database/redis/aliyun-hotkey-proxy-query-cache.png)通过阿里云的Proxy Query Cache优化热点Key问题
...
**参考答案**：[Redis 内存碎片详解open in new window](https://javaguide.cn/database/redis/redis-memory-fragmentation.html)。

## [#](#redis-生产问题-重要) Redis 生产问题（重要）
...
![缓存穿透](https://oss.javaguide.cn/github/javaguide/database/redis/redis-cache-penetration.png)缓存穿透
...
![加入布隆过滤器之后的缓存处理流程图](https://oss.javaguide.cn/github/javaguide/database/redis/redis-cache-penetration-bloom-filter.png)加入布隆过滤器之后的缓存处理流程图
...
![缓存击穿](https://oss.javaguide.cn/github/javaguide/database/redis/redis-cache-breakdown.png)缓存击穿
...
![缓存雪崩](https://oss.javaguide.cn/github/javaguide/database/redis/redis-cache-avalanche.png)缓存雪崩
...
### [#](#哪些情况可能会导致-redis-阻塞) 哪些情况可能会导致 Redis 阻塞？

单独抽了一篇文章来总结可能会导致 Redis 阻塞的情况：[Redis 常见阻塞原因总结open in new window](https://javaguide.cn/database/redis/redis-common-blocking-problems-summary.html)。
...
**参考答案**：[Redis 集群详解（付费）open in new window](https://javaguide.cn/database/redis/redis-cluster.html)。

## [#](#redis-使用规范) Redis 使用规范
...
著作权归Guide所有 原文链接：https://javaguide.cn/database/redis/redis-questions-02.html#redis-%E9%9B%86%E7%BE%A4