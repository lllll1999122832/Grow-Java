...
[Redisopen in new window](https://redis.io/) 是一个基于 C 语言开发的开源数据库（BSD 许可），与传统数据库不同的是 Redis 的数据是存在内存中的（内存数据库），读写速度非常快，被广泛应用于缓存方向。并且，Redis 存储的是 KV 键值对数据。
...
个人学习的话，你可以自己本机安装 Redis 或者通过 Redis 官网提供的[在线 Redis 环境open in new window](https://try.redis.io/)（少部分命令无法使用）来实际体验 Redis。

![try-redis](https://oss.javaguide.cn/github/javaguide/database/redis/try.redis.io.png)try-redis

全世界有非常多的网站使用到了 Redis ，[techstacks.ioopen in new window](https://techstacks.io/) 专门维护了一个[使用 Redis 的热门站点列表open in new window](https://techstacks.io/tech/redis) ，感兴趣的话可以看看。
...
下面这张图片总结的挺不错的，分享一下，出自 [Why is Redis so fast?open in new window](https://twitter.com/alexxubyte/status/1498703822528544770) 。

![why-redis-so-fast](E:/assets/why-redis-so-fast-d3507ae8.png)why-redis-so-fast
...
另外，腾讯也开源了一款类似于 Redis 的分布式高性能 KV 存储数据库，基于知名的开源项目 [RocksDBopen in new window](https://github.com/facebook/rocksdb) 作为存储引擎 ，100% 兼容 Redis 协议和 Redis4.0 所有数据模型，名为 [Tendisopen in new window](https://github.com/Tencent/Tendis)。

关于 Redis 和 Tendis 的对比，腾讯官方曾经发过一篇文章：[Redis vs Tendis：冷热混合存储版架构揭秘open in new window](https://mp.weixin.qq.com/s/MeYkfOIdnU6LYlsGb24KjQ) ，可以简单参考一下。
...
### [#](#常见的缓存读写策略有哪些) 常见的缓存读写策略有哪些？

关于常见的缓存读写策略的详细介绍，可以看我写的这篇文章：[3 种常用的缓存读写策略详解open in new window](https://javaguide.cn/database/redis/3-commonly-used-cache-read-and-write-strategies.html) 。
...
### [#](#如何基于-redis-实现分布式锁) 如何基于 Redis 实现分布式锁？

关于 Redis 实现分布式锁的详细介绍，可以看我写的这篇文章：[分布式锁详解open in new window](https://javaguide.cn/distributed-system/distributed-lock.html) 。
...
![pub/sub 实现消息队列演示](https://oss.javaguide.cn/github/javaguide/database/redis/redis-pubsub-message-queue.png)pub/sub 实现消息队列演示
...
相关阅读：[Redis 消息队列发展历程 - 阿里开发者 - 2022open in new window](https://mp.weixin.qq.com/s/gCUT5TcCQRAxYkTJfTRjJw)。

## [#](#redis-数据结构) Redis 数据结构

> 关于 Redis 5 种基础数据结构和 3 种特殊数据结构的详细介绍请看下面这两篇文章：
>
> - [Redis 5 种基本数据结构详解open in new window](https://javaguide.cn/database/redis/redis-data-structures-01.html)
> - [Redis 3 种特殊数据结构详解open in new window](https://javaguide.cn/database/redis/redis-data-structures-02.html)
...
关于 String 的详细介绍请看这篇文章：[Redis 5 种基本数据结构详解open in new window](https://javaguide.cn/database/redis/redis-data-structures-01.html)。
...
### [#](#string-的底层实现是什么) String 的底层实现是什么？
...
![Hash维护简单的购物车信息](https://oss.javaguide.cn/github/javaguide/database/redis/hash-shopping-cart.png)Hash维护简单的购物车信息
...
![img](https://oss.javaguide.cn/github/javaguide/database/redis/2021060714195385.png)

[《Java 面试指北》open in new window](https://javaguide.cn/zhuanlan/java-mian-shi-zhi-bei.html) 的「技术面试题篇」就有一篇文章详细介绍如何使用 Sorted Set 来设计制作一个排行榜，感兴趣的小伙伴可以看看。

![img](https://oss.javaguide.cn/github/javaguide/database/redis/image-20220719071115140.png)
...
![img](https://oss.javaguide.cn/github/javaguide/database/redis/image-20220720194154133.png)img
...
Redis 持久化机制（RDB 持久化、AOF 持久化、RDB 和 AOF 的混合持久化） 相关的问题比较多，也比较重要，于是我单独抽了一篇文章来总结 Redis 持久化机制相关的知识点和问题：[Redis 持久化机制详解open in new window](https://javaguide.cn/database/redis/redis-persistence.html) 。

## [#](#redis-线程模型-重要) Redis 线程模型（重要）

对于读写命令来说，Redis 一直是单线程模型。不过，在 Redis 4.0 版本之后引入了多线程来执行一些大键值对的异步删除操作， Redis 6.0 版本之后引入了多线程来处理网络请求（提高网络 IO 读写性能）。

### [#](#redis-单线程模型了解吗) Redis 单线程模型了解吗？
...
![文件事件处理器（file event handler）](https://oss.javaguide.cn/github/javaguide/database/redis/redis-event-handler.png)文件事件处理器（file event handler）

相关阅读：[Redis 事件机制详解open in new window](http://remcarpediem.net/article/1aa2da89/) 。
...
![redis4.0 more thread](https://oss.javaguide.cn/github/javaguide/database/redis/redis4.0-more-thread.png)redis4.0 more thread
...
相关阅读：[为什么 Redis 选择单线程模型？open in new window](https://draveness.me/whys-the-design-redis-single-thread/) 。

### [#](#redis6-0-之后为何引入了多线程) Redis6.0 之后为何引入了多线程？
...
关于 Redis 后台线程的详细介绍可以查看 [Redis 6.0 后台线程有哪些？open in new window](https://juejin.cn/post/7102780434739626014) 这篇就文章。

## [#](#redis-内存管理) Redis 内存管理

### [#](#redis-给缓存数据设置过期时间有啥用) Redis 给缓存数据设置过期时间有啥用？
...
![redis过期字典](https://oss.javaguide.cn/github/javaguide/database/redis/redis-expired-dictionary.png)redis过期字典
...
### [#](#redis-内存淘汰机制了解么) Redis 内存淘汰机制了解么？
